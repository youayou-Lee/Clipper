...
> `defense-mapping.md`(Issue #26 产出)。
> 红线:产物为文档,不含可运行恶意代码,不针对真实目标;所有论断带出处,推断处标注〔推断〕。
> **实验室实践边界**:本项目允许的动手实践仅限**本机与自有局域网内的自有测试机**(you-win,见
> Issue #38 演练实验室),载荷使用本仓库自身的无害 clipper;本文档本身保持威胁情报定位,
> 不提供可操作的攻击投放教程(钓鱼/链接/二维码等分发现场只做特征级归纳)。
